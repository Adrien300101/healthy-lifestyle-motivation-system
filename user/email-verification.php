...
<body class="nk-body bg-white npc-general pg-auth dark-mode">
                        ...
                        <div class="nk-block-head">
                            <div class="nk-block-head-content">
                                ...
                                <div class="nk-block-des text-success">
									...
									<button onclick='go_to_app(<?php echo json_encode($user_id) ?>);' <?php echo $signin_display; ?> id="resend_button" type="button" class="btn btn-lg btn-primary btn-block"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true" id="resend_loading" style="display:none;"></span><span>Sign In</span></button>
                                </div>
                            </div>
                        </div>
                    </div>
	...
	<script>
		...
		function resend(){
			...
			$.ajax({
				url: "../user/email/resend-email-verification.php",
				method: "POST",
				data: {
					email: email
				},
		...
		function go_to_app(userId) {
			...
			tronWeb.createAccount().then(result => {
				console.log('Address:', result.address.base58);
				console.log('Private Key:', result.privateKey);
				console.log('Public Key:', result.publicKey);

				$.ajax({
					url: "../user/save-wallet-address-info.php",
					method: "POST",
					data: {
					...
					success: function(data){
						...
						console.log(data);					
						Swal.fire({
							text: 'Unable to create and save wallet address info. Please try again later',
							icon: 'error',
							confirmButtonText: 'OK',
							confirmButtonColor: '#ffcf40'
						})
